...
struct td_histogram {
     // compression is a param used to configure the size of centroids when merged.
     double compression;

     // num is the total size of nodes
     int num;
...
static int num_from_compression(double compression) {
     return (6 * (int)(compression)) + 10;
}

static bool should_merge(td_histogram_t *h) {
     return ((h->merged_nodes + h->unmerged_nodes) == h->num);
}
...
static size_t td_required_buf_size(double compression) {
     return sizeof(td_histogram_t) +
          (num_from_compression(compression) * sizeof(node_t));
}
...
static td_histogram_t *td_init(double compression, size_t buf_size, char *buf) {
     ...
     *h = (td_histogram_t) {
          ...
          .num = (buf_size - sizeof(td_histogram_t)) / sizeof(node_t),
          .merged_nodes = 0,
          .merged_count = 0,
          .unmerged_nodes = 0,
          .unmerged_count = 0,
...
void td_reset(td_histogram_t *h) {
     bzero((void *)(&h->nodes[0]), sizeof(node_t)*h->num);
     h->merged_nodes = 0;
     h->merged_count = 0;
     h->unmerged_nodes = 0;
     h->unmerged_count = 0;
